...
fn main() {
    ...
    for s in contents.split("\r\n") {
        ...
        let is_containing = is_range_containing_range(&elf1_range, &elf2_range);
        if is_containing { total = total + 1 ;}
        println!("Elf1 range: {:?}, Elf2 range: {:?}, one contains other: {}", elf1_range, elf2_range, is_containing);
    }
